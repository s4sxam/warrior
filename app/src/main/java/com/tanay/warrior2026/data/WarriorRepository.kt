...
// [NEW]    v4.3.0: SEEN_ACHIEVEMENT_IDS key — a comma-joined set of achievement
//                  IDs whose one-time unlock animation has already played.
//                  Kept separate from WarriorState/warriorStateFlow on purpose:
//                  achievement unlock/progress itself is always DERIVED from
//                  real habit data (see data/Achievements.kt) — this key only
//                  tracks a UI concern (has the reveal animation played yet),
//                  not game state, so it doesn't belong on WarriorState.
...
class WarriorRepository(private val context: Context) {
    ...
    private object Keys {
        ...
        // v4.3.0 achievements — which unlock animations have already played
        val SEEN_ACHIEVEMENT_IDS = stringPreferencesKey("w_seen_achievements_v9")
    }
    ...
    // ── v4.3.0: Achievements — "seen unlock animation" tracking ─────────────
    //
    // Achievement IDs (e.g. "streak_7") never contain commas, so a simple
    // comma-joined string is safe here — no need for full JSON encoding for
    // what's just a set of short, comma-safe tokens.

    /** Returns the set of achievement IDs whose unlock reveal has already played. */
    suspend fun getSeenAchievementIds(): Set<String> {
        val raw = context.dataStore.data.first()[Keys.SEEN_ACHIEVEMENT_IDS] ?: ""
        return if (raw.isBlank()) emptySet() else raw.split(",").filter { it.isNotBlank() }.toSet()
    }

    /** Persists the full updated set of "already seen" achievement IDs. */
    suspend fun saveSeenAchievementIds(ids: Set<String>) {
        context.dataStore.edit { prefs ->
            prefs[Keys.SEEN_ACHIEVEMENT_IDS] = ids.joinToString(",")
        }
    }
